...
const int LED_PIN_14 = 14;
// Change listener path to the parent directory
const char *listenerPath = "board1/outputs/digital";
...
void processData(AsyncResult &aResult){
  ...
  if (aResult.available()) {
    ...
    String subPath = result.dataPath().c_str(); //capture the name
    String val = result.to<String>(); //capture the value
    val.trim();

    Serial.print("Data event: ");
    Serial.println(val);

    if (subPath == "/12") digitalWrite(LED_PIN_12, (val == "1" || val == "true") ? HIGH : LOW);
    else if (subPath == "/13") digitalWrite(LED_PIN_13, (val == "1" || val == "true") ? HIGH : LOW);
    else if (subPath == "/14") digitalWrite(LED_PIN_14, (val == "1" || val == "true") ? HIGH : LOW);
  }
}